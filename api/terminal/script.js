...
// Registry of launchable units (games, tools, admin pages) that live as peer
// folders under the flat apps/ directory (see manifest.json contract).
// Adjust `path` below if a given app lives somewhere else.
const APPS_ = [
// Games
{ command: 'pacman', title: 'Pac-Man', path: 'apps/pacman/', category: 'Games' },
{ command: 'dino', title: 'Dino', path: 'apps/dino/', category: 'Games' },
{ command: 'dino3d', title: 'Dino 3D', path: 'apps/dino3d/', category: 'Games' },
{ command: 'pong', title: 'Pong', path: 'apps/pong/', category: 'Games' },
{ command: 'rubik', title: 'Rubik', path: 'apps/rubik/', category: 'Games' },
{ command: 'blocks', title: 'Blocks', path: 'apps/blocks/', category: 'Games' },
{ command: 'crossy', title: 'Crossy', path: 'apps/crossy/', category: 'Games' },
{ command: 'sticky', title: 'Sticky', path: 'apps/sticky/', category: 'Games' },
{ command: 'rabbit', title: 'Rabbit', path: 'apps/rabbit/', category: 'Games' },
{ command: 'snake', title: 'Snake', path: 'apps/snake/', category: 'Games' },
// Tools
{ command: 'currency', title: 'Currency', path: 'apps/currency/', category: 'Tools' },
{ command: 'crypto', title: 'Crypto', path: 'apps/crypto/', category: 'Tools' },
{ command: 'domain', title: 'Domain', path: 'apps/domain/', category: 'Tools' },
{ command: 'editor', title: 'Editor', path: 'apps/editor/', category: 'Tools' },
{ command: 'donate', title: 'Donate', path: 'apps/donate/', category: 'Tools' },
{ command: 'qr', title: 'QR', path: 'apps/qr/', category: 'Tools' },
{ command: 'math', title: 'Math', path: 'apps/math/', category: 'Tools' },
// Admin
{ command: 'resume', title: 'Resume', path: 'apps/resume/', category: 'Admin' },
{ command: 'bday', title: 'Bday', path: 'apps/bday/', category: 'Admin', aliases: ['nofap'] },
// Misc pages
{ command: 'store', title: 'Store', path: 'apps/store/', category: 'Pages' },
{ command: 'v2', title: 'v2', path: 'apps/v2/', category: 'Pages' },
{ command: 'gtin', title: 'GTIN', path: 'apps/gtin/', category: 'Pages' }];
...
class Field extends React.Component {
  constructor(props) {
    ...
    {
      command: 'echo',
      purpose: 'Prints back whatever text follows it.',
      help: [
      'ECHO <TEXT>',
      'Prints back whatever text follows it.'] },

    {
      command: 'whoami',
      purpose: 'Displays the current user.' },
    {
      command: 'uname',
      purpose: 'Displays browser/system information.' },
    {
      command: 'cat',
      purpose: 'Fetches a URL and prints its contents.',
      help: [
      'CAT <URL>',
      'Fetches a URL and prints its raw contents.',
      'Note: only works against URLs that allow cross-origin requests.'] },
    {
      command: 'start',
    ...
    ...APPS_.map(app => ({
      command: app.command,
      purpose: `Launches ${app.title} (${app.category}).` })),

    {
      command: 'close',
      purpose: 'Closes the currently open app/game window.' },
    {
      command: 'docs',
      purpose: 'Opens the documentation site in a new tab.' },
  ...
  handleInputExecution(cmd, params = [], flags = []) {
    if (cmd === 'help') {
      ...
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, {
          ...
          '',
          'Games:',
          ...APPS_.filter(a => a.category === 'Games').map(a => a.command.toUpperCase()),
          '',
          'Tools:',
          ...APPS_.filter(a => a.category === 'Tools').map(a => a.command.toUpperCase()),
          '',
          'Admin:',
          ...APPS_.filter(a => a.category === 'Admin').map(a => a.command.toUpperCase()),
          '',
      ...
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: params.join(' '), hasBuffer: true }] }));

    } else if (cmd === 'whoami') {
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: 'user', hasBuffer: true }] }));

    } else if (cmd === 'uname') {
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: navigator.appVersion, hasBuffer: true }] }));

    } else if (cmd === 'cat') {
      if (params.length !== 1) {
        return this.setState(state => ({
          fieldHistory: [...state.fieldHistory, this.giveError('bp', { cmd: 'CAT', noAccepted: 1 })] }));

      }

      const url = /http/i.test(params[0]) ? params[0] : `https://${params[0]}`;

      fetch(url).
      then(res => res.text()).
      then(text => this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text, hasBuffer: true }] }))).
      catch(() => this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: `Could not fetch ${url}.`, isError: true, hasBuffer: true }] })));

      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: `Fetching ${url}...`, hasBuffer: true }] }));
    ...
    } else if (APPS_.some(a => a.command === cmd || (a.aliases && a.aliases.includes(cmd)))) {
      const app = APPS_.find(a => a.command === cmd || (a.aliases && a.aliases.includes(cmd)));

      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: `Launching ${app.title}...`, hasBuffer: true }],
        activeApp: app }));

    } else if (cmd === 'close') {
      if (!this.state.activeApp) {
        return this.setState(state => ({
          fieldHistory: [...state.fieldHistory, { text: 'No app is currently open.', hasBuffer: true }] }));

      }

      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: `Closed ${state.activeApp.title}.`, hasBuffer: true }],
        activeApp: null }));

    } else if (cmd === 'docs') {
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: 'Opening docs in a new window...', hasBuffer: true }] }),
      () => window.open('https://aliev.io/docs', '_blank'));
    } else if (cmd === 'exit') {
  ...
  render() {
    ...
    const { fieldHistory, userInput, activeApp } = this.state;
    ...
    activeApp && /*#__PURE__*/React.createElement(AppFrame, {
      app: activeApp,
      onClose: () => this.setState({ activeApp: null }) }));

  }}

const AppFrame = ({ app, onClose }) => /*#__PURE__*/React.createElement("div", { id: "app-frame" }, /*#__PURE__*/
React.createElement("div", { id: "app-frame-bar" }, /*#__PURE__*/
React.createElement("span", { id: "app-frame-title" }, app.title), /*#__PURE__*/
React.createElement("button", { id: "app-frame-close", onClick: onClose }, "\u2715 CLOSE (Esc)")), /*#__PURE__*/

React.createElement("iframe", { id: "app-frame-iframe", src: app.path, title: app.title, frameBorder: "0" }));
